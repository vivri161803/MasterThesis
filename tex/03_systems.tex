...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=1\linewidth]{3/Gemini_Generated_Image_2t6bs42t6bs42t6b.jpg}
  \caption{The extraction pipeline. Each block narrows what the next one is allowed to consider: Block~1 decides which passages matter, Block~2 which schema applies, Block~3 which fields that schema contains. The two gates marked in the figure are the points at which an unsupported value is refused.}
...
The service ships as a single multi-stage Docker image (figure~\ref{fig:bulkcontainer}), roughly 2.5~GB, CPU-only \citep{merkel2014docker}. The embedding model and the \texttt{contracts/} directory of 97 per-leaf JSON schema files are baked into the image rather than fetched at startup, so a container reaches readiness (\S\ref{sec:bulk-insert-api}) without any network dependency beyond the OpenRouter calls it makes per request; only the OpenRouter API key is injected at runtime. Horizontal scaling — how many replicas run behind what load balancer or autoscaler — is a deployment-platform concern outside this image's own configuration surface, consistent with the architectural choice described in \S\ref{sec:bulk-insert-api}.

\begin{figure}[htbp]
  \centering
  \includegraphics[width=1\linewidth]{3/Gemini_Generated_Image_xhfl2uxhfl2uxhfl.jpg}
  \caption{Container mapping and deployment strategy}
  \label{fig:bulkcontainer}
\end{figure}

\section{Part B --- ColdRAG}
\label{sec:coldrag-system}

ColdRAG (figure~\ref{fig:coldrag-architecture}) is the graph-augmented matching system this thesis's title refers to: a domain-specific implementation, adapted for real-estate off-market intermediation at Dometria AI, of the retrieval-augmented cold-start framework of \S\ref{sec:coldrag-paper}, with an ingestion pipeline built against the DIAL-KG framework of \S\ref{sec:dial-kg} rather than the ColdRAG paper's own graph-construction method. It comprises three modules described in turn below: an ingestion pipeline that builds the knowledge graph (\S\ref{sec:coldrag-ingestion}), an inference engine that retrieves candidates from it (\S\ref{sec:coldrag-inference}), and a conversational completion module that acquires structured input from a human when required fields are missing (\S\ref{sec:coldrag-hitl}).
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=1\linewidth]{3/Gemini_Generated_Image_vv38jbvv38jbvv38.jpg}
  \caption{ColdRAG's three modules. Ingestion and retrieval never address each other directly; the graph is the only thing they share, which is what allows a product ingested by one to be reachable by the other without a second extraction of its content.}
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=1\linewidth]{3/Gemini_Generated_Image_cmk0fhcmk0fhcmk0.jpg}
  \caption{Exposed endpoints for the ColdRAG module}
\end{figure}
