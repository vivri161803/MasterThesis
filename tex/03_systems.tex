...
Contract matching proceeds in two stages against a fixed, three-level category tree of 97 leaf contracts, spanning 3 root macro-categories and 12 intermediate categories. The first stage is a single constrained call to the lightweight LLM that selects one of the 3 root macro-categories. 

The second stage selects the (level-2, level-3) leaf. Not every chunk participates: those scoring below \texttt{CONTRACT\_MIN\_SCORE} (default 5) are discarded, and at most the \texttt{CONTRACT\_TOP\_K} (default 5) highest-scoring of the remainder are retained, so the vote is taken over the passages Block~1 judged informative rather than over the whole document. Each retained chunk is embedded with the multilingual, distilled sentence-embedding model \texttt{paraphrase-multilingual-MiniLM-L12-v2} (384-dimensional; \S\ref{sec:embedding-models}) \citep{reimers2026sentence} and compared by cosine similarity against the embeddings of the candidate leaf labels, each label rendered as the Italian string \emph{level-2 name}~\texttt{>}~\emph{level-3 name}. A chunk casts a single vote for its highest-similarity label, carrying its Block~1 relevance score as the vote's weight; the leaf with the greatest cumulative weight wins, ties broken by the highest single-chunk similarity any one chunk gave it. 

The tree itself — root, intermediate, and leaf categories, and the ancestry between them — is a plain nested dictionary traversed at lookup time. An earlier design used a dedicated knowledge-graph engine for this matching step; it was removed as disproportionate to a fixed, shallow, three-level structure that a nested-dictionary lookup expresses directly, and no traversal beyond ancestor/descendant lookup was ever required of it.
...
Finally, values that require computation rather than transcription are never requested from the extraction model. Arithmetic and cross-field values are computed in Python from already-transcribed primitives (\texttt{computed} fields); a small number of fields that genuinely require reasoning over already-extracted content (\texttt{judged} fields) are resolved with an isolated second LLM call that receives only the relevant primitives and their quotes, never the source document. Where a \texttt{computed} field's target was also extracted directly from the text, a reconciliation pass cross-checks the two and raises a flag on disagreement, which is a consistency check obtained at no additional model cost. Both derivation passes are implemented but inert on the artifacts the service currently ships: all 1083 fields in the metadata sidecar carry a null derivation rule, so no field is computed or judged until an offline step populates one. The mechanism is in place; the rules are not yet written. The count/aggregate rule of \S\ref{sec:bulk-insert-normalization} is seeded independently of this and does fire, on the 175 fields that declare one. Field metadata — enum vocabularies, descriptions, topics, priorities, and derivation rules — lives in an editable \texttt{field\_metadata.yaml} sidecar populated offline (\S\ref{sec:bulk-insert-metadata-compile}); a field not yet compiled is treated as free text pending compilation.


\begin{figure}[htbp]
\centering
\figplaceholder[0.34\textheight]{Figure placeholder --- the
\texttt{dometria-bulk-insert} block chain. A left-to-right flow:
\{id, text\} $\rightarrow$ \textbf{Block 1} (spaCy sentencizer $\rightarrow$
sliding window, 6 sentences with 2 overlapping $\rightarrow$ one lightweight-LLM
call per chunk returning a 1--10 relevance score and the coreference-resolved
text) $\rightarrow$ \textbf{Block 2} (chunks filtered at score $\geq 5$ and
cut to the top 5; a constrained lightweight call fixes the level-1
macro-category; weighted cosine vote over the candidate level-2/level-3 labels
picks the leaf) $\rightarrow$ \textbf{Block 3} (the leaf selects its fields
from the offline field tree; \texttt{create\_model} builds the Pydantic schema;
per-topic extraction calls to the heavy model, with self-consistency voting on
the mandatory and high-priority groups) $\rightarrow$ \textbf{finishing}
(normalization, \texttt{OUT\_OF\_VOCAB} resolution, count/aggregate,
derivation) $\rightarrow$ \textbf{grounding gate} (batched entailment judge)
$\rightarrow$ \{asset, confidence, completeness\}. Mark the two in-band gates
distinctly: the quote validator inside Block 3, which loops back to Block 3 as
an Instructor retry, and the grounding gate, whose failures null a field rather
than retrying it. Mark the offline artifacts --- field tree, product manifests,
metadata sidecar, topic table --- as inputs read at request time but built by a
separate build-time path.}
\caption{The extraction pipeline. Each block narrows what the next one is
allowed to consider: Block~1 decides which passages matter, Block~2 which
schema applies, Block~3 which fields that schema contains. The two gates marked
in the figure are the points at which an unsupported value is refused.}
\label{fig:bulk-insert-pipeline}
\end{figure}
...
\subsection{The grounding gate, confidence, and completeness}
\label{sec:bulk-insert-grounding}

The validator of \S\ref{sec:bulk-insert-block3} enforces that a populated field \emph{has} a quote; it cannot tell whether the quote supports the value, because it is ordinary Python running against a declared model and has no access to meaning. A separate final pass supplies that judgement. Every populated, evidence-bearing, non-derived field is submitted to a single batched LLM-as-judge call over the heavy model, which returns one pass/fail verdict per field on whether the quote, read in the context of the document, entails the value (\S\ref{sec:llm-as-judge}). A field that fails is nulled and flagged \texttt{ungrounded} rather than silently kept.

The judge is instructed to make one exception, and it is a domain-driven one. Italian property descriptions state magnitudes as bounds and approximations — \emph{oltre 5.000}, \emph{circa 200}, \emph{almeno 3} — so a numeric value transcribed faithfully from such a phrase would fail a literal entailment test despite being the correct reading. Numbers are therefore accepted when the quote states the magnitude as a bound or an approximation; enum, boolean and free-text values must still be supported literally. The exception is confined to numbers precisely because that is where the mismatch between faithful transcription and literal entailment arises.

The two scores the service returns are computed from this pass rather than asserted by the model. \emph{Completeness} is the fraction of mandatory fields that ended up filled. \emph{Confidence} is the product of the grounding pass-rate and that completeness, so a record can lose confidence either by failing entailment on what it did extract or by not extracting what it was required to. Any mandatory field still null is appended to the \texttt{review\_queue} of \S\ref{sec:bulk-insert-api}. Low confidence never raises: the gate is fail-safe, and a poorly grounded record is returned, marked, and routed to a human rather than rejected.
...
A greedy nearest-representative pass was tried first and rejected: a generic, high-frequency name (\emph{superfici}, ``surfaces'') acted as a magnet under that scheme, absorbing 56 unrelated topics into a single hub cluster. Agglomerative clustering does not privilege whichever cluster happens to be nearest at each step in this way, and was adopted for that reason. Three clustering thresholds were swept: 0.55 yields 50 clusters (largest, 56 members); 0.62 yields 74 clusters (largest, 25 members); 0.68 yields 95 clusters (largest, 17 members) — the threshold trades cluster count against residual fragmentation, and is exposed as \texttt{TOPIC\_CLUSTER\_THRESHOLD} (default 0.6) alongside \texttt{TOPIC\_CLUSTER\_ENABLED} (default enabled) and \texttt{TOPIC\_CLUSTER\_LINKAGE} (default \texttt{average}), all CLI-overridable. 

The artifacts the service ships were produced at the default threshold and carry 64 representative topics over the 1083 fields. Consolidation reduced the count by roughly a factor of four, but it did not equalize the groups: the largest cluster, \emph{superfici}, holds 335 fields, just under a third of the field set, and fields with little to do with surfaces are grouped under it. Since topic membership is what partitions extraction into per-call groups (\S\ref{sec:bulk-insert-block3}), a cluster of that size returns the largest group to roughly the schema size that partitioning was introduced to avoid (\S\ref{sec:structured-output}). The hub effect that motivated abandoning the greedy pass is reduced by agglomerative clustering rather than eliminated by it. Cluster identifiers are derived from a specific run of the clustering algorithm and are documented as such: they are not guaranteed stable if the pass is rerun, and \texttt{topics.yaml}, \texttt{metadata.py}, and the topic-based partitioning of \S\ref{sec:bulk-insert-block3} consume the resulting name/id table unchanged, unaware that consolidation happened beneath it.
...
Ingestion (\texttt{kg\_builder.py}, \texttt{ingest.py}) begins by computing a deterministic identifier for the incoming document as the SHA-256 hash of its text, and creating or matching a Product node against that identifier — the same document text always yields the same product node, so re-ingesting a document already in the graph updates it rather than duplicating it. 

An LLM call extracts the document's leaf-level entities and its product title. The extraction schema constrains what may be returned: an entity's type is a closed vocabulary of \texttt{feature}, \texttt{genre}, \texttt{target\_user} and \texttt{setting}, and the model is instructed that it may not emit the product itself as an entity. Numeric and categorical attributes of the product are collected separately into a metadata dictionary rather than promoted to nodes, which keeps quantities such as price and surface area as properties of one node instead of scattering them across the graph as entities in their own right. 

This separation is DIAL-KG's \emph{dual-track extraction} (\S\ref{sec:dial-kg}) as realized here: one track yields the graph-worthy entities and the relations among them, the other the product-level identifying record, and the two are validated and written by different means — the entity track through disambiguation and governance below, the product track deterministically from the document hash. Keeping them apart is also what makes the immunity invariant stated below expressible at all, since a product never enters the pipeline that merges entities. Each extracted entity is embedded with BGE-M3, a multilingual, multi-granularity, multi-function embedding model producing 1024-dimensional vectors (\S\ref{sec:embedding-models}) \citep{chen2024m3}.

Coreference alignment and named entity disambiguation (\S\ref{sec:ned}) follow the two-stage pattern of vector retrieval plus LLM adjudication: a Neo4j native vector search proposes a small candidate set of already-graphed entities similar to the new one, and an LLM judge decides which candidate, if any, the new entity should be merged into. The vector stage is bounded by \texttt{COLDRAG\_NED\_TOP\_K} (default 10) candidates above a cosine floor of \texttt{COLDRAG\_NED\_THRESHOLD} (default 0.75); when it proposes nothing, the judge is not called at all and the entity keeps its own name. The judge's response model admits three verdicts — merge, hierarchy, and separate — but only merge changes what is written: a hierarchy verdict is recorded and then treated exactly as a separate one, so the relation it was meant to express does not reach the graph. The three-way vocabulary is therefore effectively binary in the current implementation. A specific invariant — \emph{NED immunity} — is enforced throughout this stage: Product nodes and item-level entity nodes are never merged into one another, regardless of embedding similarity, because a product and the entities it mentions are categorically distinct in this graph and conflating them would corrupt both the document-provenance role Product nodes play and the entity-level structure item nodes play.

Every candidate merge and every candidate new edge then passes through a governance adjudication layer before it is committed, applying the entailment-gating pattern of \S\ref{sec:llm-as-judge} at two checks: an evidence check, verifying the claim is actually supported by the source text, and a logical check, verifying it does not contradict what the graph already asserts. 

Governance is conservative by design, and the instruction says so in as many words: the evidence judge is told to reject a fact \emph{only} if the source text actively contradicts it, and to accept anything the text does not actively disprove. Two qualifications belong with that claim. The conservatism is a property of the prompt, whereas the surrounding code is fail-closed — a fact survives only if the judge returns it as valid, so a fact the judge omits from its response is dropped rather than admitted. And verdicts are cached by a hash of the fact, so a fact already adjudicated once in the run is accepted on the strength of that earlier verdict without being re-examined. Subject to those, recall is favored over precision at this gate specifically, on the reasoning that an ingestion pipeline that runs repeatedly over a large corpus can afford to admit a supportable fact and revisit it later, but cannot cheaply undo a graph corrupted by an over-eager rejection rule that silently drops valid structure. 
...
\begin{figure}[htbp]
\centering
\figplaceholder[0.34\textheight]{Figure placeholder --- the three ColdRAG
modules against the single Neo4j store. Centre: Neo4j, holding product nodes,
entity nodes, the relations between them, the vector index over entity
embeddings, and the singleton Meta-Knowledge Base. Left, \textbf{ingestion}:
document $\rightarrow$ SHA-256 product id $\rightarrow$ LLM extraction split
into the two tracks (entities and relations; product title and metadata)
$\rightarrow$ BGE-M3 embeddings $\rightarrow$ NED (vector candidates, then an
LLM judge) $\rightarrow$ governance (evidence check, then logical check)
$\rightarrow$ batched \texttt{UNWIND}/\texttt{MERGE} write. Right,
\textbf{retrieval}: query $\rightarrow$ LLM analysis into key concepts and the
\texttt{needs\_reasoning} flag $\rightarrow$ hybrid router $\rightarrow$
either direct vector search or the multi-hop loop (fan-out, edge grouping,
0--10 scoring, pruning at $\gamma$) $\rightarrow$ top-$k$ blind-match ranking
$\rightarrow$ Laplace perturbation applied only on the outbound edge, which
should be drawn at the system boundary rather than inside the loop. Bottom,
\textbf{conversational completion}: the taxonomy-driven state machine, with
session state in Redis and its confirmed payload routed back to either
ingestion or retrieval.}
\caption{ColdRAG's three modules. Ingestion and retrieval never address each
other directly; the graph is the only thing they share, which is what allows a
product ingested by one to be reachable by the other without a second
extraction of its content.}
\label{fig:coldrag-architecture}
\end{figure}
...
The multi-hop loop begins from a hop-0 frontier of \texttt{COLDRAG\_HOP0\_FRONTIER} (default 5) nodes returned by a Neo4j native vector search against the query embedding. Each subsequent hop issues a fan-out Cypher query expanding the current frontier by one edge in every direction, groups the resulting edges to compress what would otherwise be repeated context by roughly 60\%, and scores each grouped edge with an LLM on a 0--10 scale (\S\ref{sec:llm-as-judge}). Edges scoring below the threshold \texttt{COLDRAG\_SCORE\_THRESHOLD} ($\gamma$, default 7.0) are pruned; the surviving frontier carries into the next hop, up to \texttt{COLDRAG\_MAX\_HOPS} (default 5) or until \texttt{COLDRAG\_TARGET\_CANDIDATES} (default 20) candidates have accumulated, whichever comes first. 

Only product nodes are promoted to candidates; entity nodes are traversed but never returned. A candidate's score is the maximum score of any single edge that reached it, not a function of the path as a whole, so depth neither improves nor penalizes a candidate — a product found three hops out through edges each scoring 7.5 and one found immediately through a single edge scoring 7.5 are indistinguishable at ranking time. Products already present in the hop-0 frontier bypass edge scoring entirely and are entered at the maximum score of 10, on the reasoning that a direct vector match needs no relational justification. Of the candidates accumulated, only the top \texttt{COLDRAG\_TOP\_K} (default 5) are carried into the final ranking call. 

Once retrieval stops, a final LLM pass performs \emph{blind matching} (\S\ref{sec:blind-matching}): candidates are ranked without exposing identifying attributes, and the ranking is returned together with a justification for the ordering. Results are streamed to the frontend progressively over Server-Sent Events as each hop completes \citep{whatwg2026server}, rather than withheld until the full loop terminates.

\subsection{Privacy layer}
\label{sec:coldrag-privacy}

Before any candidate's numeric metadata is exposed to a user, it is perturbed with the Laplace mechanism of \S\ref{sec:dp-definition}, at privacy budget \texttt{COLDRAG\_DP\_EPSILON} (default 1.0). Numeric properties are perturbed and rounded; strings and booleans pass through unchanged, and the structural keys identifying the node are dropped from the payload rather than noised.

One departure from the definition of \S\ref{sec:dp-definition} must be stated plainly, because it bounds what the mechanism here actually guarantees. Equation~\ref{eq:sensitivity} defines sensitivity $\Delta f$ as a property of the query over neighbouring datasets: the largest change any single record can force in the output. The implementation instead sets the sensitivity of each released value to ten per cent of that value, so the noise scale is derived from the quantity being protected rather than from the query. The consequence is that the released noise distribution is itself a function of the true value, and the bound of Equation~\ref{eq:dp} does not follow from this construction. What the layer provides is calibrated obfuscation proportional to magnitude, which serves the blind-matching requirement of \S\ref{sec:blind-matching} by coarsening a figure before a counterparty sees it; it is not a proof of $\varepsilon$-differential privacy in the sense \S\ref{sec:dp-definition} defines, and this thesis does not claim it as one. This is applied at the point of exposure, after ranking, so the values used internally for scoring and traversal are exact; only what leaves the system toward the frontend is perturbed. \S\ref{sec:blind-matching} explains why this is a domain requirement of off-market intermediation rather than an incidental hardening measure: numeric attributes precise enough to support matching are also precise enough to re-identify a listing if exposed repeatedly across queries, which is exactly the cumulative risk a formal privacy budget bounds.
...
A \texttt{PerformanceTracker} (\texttt{metrics.py}) instruments every request: latency, an estimated token count (a length-based heuristic, characters divided by four), an estimated dollar cost from a configurable per-million-token input/output price (defaulting to \$0.30 in / \$2.50 out), and throughput in tokens per second. \S\ref{sec:conclusions-results} reports the tool and one illustrative run it produced; neither figure is a claim about model quality or recommendation accuracy.
